...
export class Http2IoT {
   ...
   */
  private request(request: IHttp2IoTReq): Promise<any> {
    const clearAuth = () => {
      Values.Token = null;
      Values.Refresh = null;
      localStorage.clear();
    ...
    const urlToCall: string = request.isAuth
      ? Constants.Settings.SERVER_AUTH + request.path
      ...
      this.http.request(request.method, urlToCall, options).subscribe(
        (response) => {
          const sendResult = (res: string) => {
            ...
            const headersToAdd = [ESHeaders.APPLICATION_ID];
            if (headers) {
   ...
   */
  public get(request: IHttp2IoTReq): Promise<any> {
    request.method = 'GET';
   ...
   * @returns Formatted request with our parameters from settings and needed headers
   */
  public post(request: IHttp2IoTReq): Promise<any> {
    request.method = 'POST';
    return this.request(request);
  }
   ...
   * @returns Formatted request with our parameters from settings and needed headers
   */
  public delete(request: IHttp2IoTReq): Promise<any> {
   ...
   * @returns Formatted request with our parameters from settings and needed headers
   */
  public patch(request: IHttp2IoTReq): Promise<any> {
    request.method = 'PATCH';
    return this.request(request);
  }
}

export interface IHttp2IoTReq {
  method?: string;
  input?: any;
  headers?: HttpHeaders;
  path?: string;
